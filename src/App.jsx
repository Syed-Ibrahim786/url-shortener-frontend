  ...
  const getShortURL = async () => {
    ...
    try {
      const data = await axios.post(
        "https://shorten-url-backend-npot.onrender.com",
        {
    ...
    <main className=" w-full h-screen  flex justify-center items-center">
      <form className=" bg-card w-[90%] mx-auto  rounded-lg p-6 flex flex-col gap-6">
        <section className=" flex flex-col gap-2">
          <label htmlFor="input" className=" text-accent">
            🔗Shorten a long URL
          </label>
          <input
            onChange={() => {
            ...
            className="text-main border-2 border-light w-full p-4 rounded-lg"
            required
          />
        </section>
        ...
        <section className=" flex flex-col gap-2">
          ...
          <input
            ...
            ref={alias}
            type="text"
            placeholder="Enter your Custom Alias     (optional)"
            className="text-main border-2 border-light w-full p-4 rounded-lg"
            required
          />
        </section>
        ...
        <section className="">
          ...
          <button
            className=" w-full bg-primary p-4 rounded-lg text-main"
            title="click for short link"
            aria-label="click to get short url"
            type="submit"
            onClick={(e) => {
              e.preventDefault();
              getShortURL();
        ...
        <section className="text-accent  flex flex-col gap-2 items-center justify-between  ">
          ...
          <section className="overflow-x-scroll w-full">
            <a
              aria-label="click for redirection"
              className=""
              href={`${domain}/${shorturl}`}
              target="_blank"
              rel="noopener noreferrer"
            >{shorturl?`${domain}/${shorturl}`:""}</a>
          </section>

          <button
            aria-label="Copy short URL"
            style={copied ? { outline: "3px solid green" } : {}}
            onClick={(e) => {
              e.preventDefault();
              copyToClipboard();
            }}
            className="flex gap-2 bg-primary-light text-main w-fit p-2 rounded-lg mt-6"
          >
            <Copy className="" /> {copied ? "copied" : "copy"}
          </button>
        </section>
      </form>
    </main>
